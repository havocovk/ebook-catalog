...
def get_indexed_paths_batch(file_paths: list) -> set:
    ...
    # ── GEÇİCİ TANI SATIRI — sorunu bulmak için, sonra kaldırılacak ─────────
    print(f"  [TANI] get_indexed_paths_batch() sorgulanan doc_id sayısı: {len(all_doc_ids)}")
    print(f"  [TANI] get_indexed_paths_batch() ilk 3 doc_id: {all_doc_ids[:3]}")
    # ── TANI SONU ─────────────────────────────────────────────────────────────
...
def save_book(metadata: dict, cover_url: str = None) -> bool:
    ...
    # ── GEÇİCİ TANI SATIRI — sorunu bulmak için, sonra kaldırılacak ─────────
    print(f"  [TANI] save_book() file_path: {repr(file_path)}")
    print(f"  [TANI] save_book() doc_id   : {doc_id}")
    # ── TANI SONU ─────────────────────────────────────────────────────────────
