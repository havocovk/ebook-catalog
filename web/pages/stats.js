...
function compute() {
  ...
  // Sayfa sayısı dağılımı — aralıklara göre
...
export async function renderStats() {
  ...
  container.innerHTML = `
    <div class="stats-wrap">
      ...
      <!-- ── Üst Özet Bar (Grimmory tarzı kompakt pill'ler) ── -->
      ...
      <!-- ── Pasta / Halka Grafikler (4'lü — Grimmory renk paleti) ── -->
  ...
  // Chart.js yükle ve grafikleri çiz — Grimmory renk paleti
  try {
    await _loadChartJs();
...
// ─── Yardımcı: Grimmory tarzı kompakt pill istatistik ────────────────────────
...
// ─── Sayfa sayısı dağılımı çubuk grafiği ─────────────────────────────────────
function _drawPageCount(s) {
  ...
  _activeCharts["chart-pagecount"] = new window.Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: { color: "#a89080", font: { size: 10 }, maxRotation: 30 },
          grid:  { color: "rgba(51,39,32,0.6)" },
        },
        y: {
          ticks: { color: "#a89080", font: { size: 10 } },
          grid:  { color: "rgba(51,39,32,0.6)" },
          beginAtZero: true,
        },
      },
...
// ═════════════════════════════════════════════════════════════════════════════
// Adım 10 — Top Yazarlar + Author Universe
// ═════════════════════════════════════════════════════════════════════════════

function computeAuthors() {
  const books = state.books;

  // ── Yazar başına kitap listesi ────────────────────────────────────────────
  const authorMap = {};
  books.forEach((b) => {
    if (!b.author) return;
    if (!authorMap[b.author]) {
      authorMap[b.author] = { name: b.author, books: [] };
    }
  ...
  // ── Top 15: kitap sayısına göre sırala ───────────────────────────────────
  const top15 = [...authors]
    .sort((a, b) => b.books.length - a.books.length)
    .slice(0, 15);

  // ── Yazar başına durum sayıları (stacked bar için) ────────────────────────
  ...
  // Yazar başına ortalama kitap sayısı
  const avgBooksPerAuthor = authors.length > 0
    ? (books.filter((b) => b.author).length / authors.length).toFixed(1)
    : 0;

  // En yüksek puanlı yazar (en az 2 kitabı olup rating'i dolu olanlar)
  const ratedAuthors = authors
    .map((a) => {
      const rated = a.books.filter((b) => b.rating);
      if (rated.length < 2) return null;
      const avg = rated.reduce((s, b) => s + b.rating, 0) / rated.length;
      return { name: a.name, avg: Math.round(avg * 10) / 10, count: a.books.length };
    })
    .filter(Boolean)
    .sort((a, b) => b.avg - a.avg);
  const topRated = ratedAuthors[0] || null;

  // En büyük backlog'u olan yazar
  const backlogAuthors = authors
    .map((a) => ({
      name:    a.name,
      backlog: a.books.filter((b) => b.status === "okunmadi" || b.status === "sirada").length,
    }))
    .sort((a, b) => b.backlog - a.backlog);
  const topBacklog = backlogAuthors[0] || null;

  // Most Versatile: en çok farklı türde kitabı olan
  const versatile = authors
    .map((a) => ({
      name:   a.name,
      genres: new Set(a.books.map((b) => b.genre).filter(Boolean)).size,
    }))
    .sort((a, b) => b.genres - a.genres);
  const topVersatile = versatile[0] || null;

  // Hiç okunmamış: tüm kitapları okunmadı/sırada olan yazarlar
  const unreadAuthors = authors.filter(
  ...
  sectionEl.innerHTML = `
    <div class="stats-section-box">
      ...
      <div class="stats-chart-legend" style="flex-direction:row;flex-wrap:wrap;gap:1rem;margin-top:0.25rem;">
        <div class="stats-legend-item">
          <span class="stats-legend-dot" style="background:rgba(92,95,114,0.85)"></span>
      ...
      <div class="stats-chart-legend" style="flex-direction:row;flex-wrap:wrap;gap:1rem;margin-top:0.25rem;">
        <div class="stats-legend-item">
          <span class="stats-legend-dot" style="background:rgba(76,175,130,0.75)"></span>
          <span class="stats-legend-label">Yüksek tamamlanma (%70+)</span>
        </div>
        <div class="stats-legend-item">
          <span class="stats-legend-dot" style="background:rgba(224,168,74,0.75)"></span>
          <span class="stats-legend-label">Orta tamamlanma (%30-69)</span>
        </div>
        <div class="stats-legend-item">
          <span class="stats-legend-dot" style="background:rgba(124,106,247,0.75)"></span>
...
function _drawTopAuthors(a) {
  ...
  const labels   = a.top15Data.map((d) => d.name);
  const unread   = a.top15Data.map((d) => d.okunmadi);
  const reading  = a.top15Data.map((d) => d.okunuyor);
  const finished = a.top15Data.map((d) => d.okundu);
  ...
  _activeCharts["chart-top-authors"] = new window.Chart(canvas, {
    ...
    data: {
      labels,
      datasets: [
        { label: "Okunmadı/Sırada", data: unread,   backgroundColor: "rgba(92,95,114,0.85)",  borderRadius: 0 },
        { label: "Okunuyor",        data: reading,  backgroundColor: "rgba(124,106,247,0.85)", borderRadius: 0 },
        { label: "Okundu",          data: finished, backgroundColor: "rgba(76,175,130,0.85)",  borderRadius: 4 },
      ],
    },
    options: {
      indexAxis: "y",
...
function _drawBubble(a) {
  ...
  const datasets = a.bubbleData.map((d, i) => ({
    label: d.label,
  ...
  _activeCharts["chart-bubble"] = new window.Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        x: {
          title: { display: true, text: "Kitap Sayısı", color: "#9195a8", font: { size: 11 } },
          ticks: { color: "#9195a8", font: { size: 11 } },
          grid:  { color: "rgba(46,50,64,0.6)" },
          beginAtZero: true,
        },
        y: {
          title: { display: true, text: "Ort. Puan", color: "#9195a8", font: { size: 11 } },
          ticks: { color: "#9195a8", font: { size: 11 } },
          grid:  { color: "rgba(46,50,64,0.4)" },
          min: 0, max: 5,
        },
...
// ─── Yardımcı: yazar özet kartı ──────────────────────────────────────────────
function authorCard(icon, label, value, variant) {
  return `
